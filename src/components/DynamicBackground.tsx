...
const DynamicBackground = ({ positioning = 'fixed' }: DynamicBackgroundProps = {}) => {
  const [theme, setTheme] = useState<Theme>('bold-tech');
  const [mode, setMode] = useState<Mode>('dark');

  // Initialize theme and mode on mount and when they change
  useEffect(() => {
    const updateTheme = () => {
      ...
      const newTheme = (themeClass.replace('theme-', '').replace('-dark', '') as Theme) || 'bold-tech';
  ...
  const backgroundMap: Record<Theme, Record<Mode, string>> = {
    ...
    'amethyst-haze': {
      ...
      dark: "absolute inset-0 -z-10 h-full w-full bg-[#1e1e1e] bg-[linear-gradient(to_right,#8080801a_1px,transparent_1px),linear-gradient(to_bottom,#8080801a_1px,transparent_1px)] bg-[size:14px_24px]"
    },
    'catppuccin': {
  ...
  return (
    <div
      className={`${positioning} inset-0 -z-10 h-full w-full transition-all duration-1000 ease-in-out ${backgroundMap[theme][mode]}`}
    >
